...
class bodyConnection(object):
    ...
    def connect(self, body1, body2=None, pt1=np.zeros(3), pt2=np.zeros(3)):
        ...
        if type(pt1) is np.ndarray:
            self.marker1 = self.body1.addMarker(
                marker('Conn_{}_marker_A'.format(self.name)))
        ...
        if body2 is not None:
            if type(pt2) is np.ndarray:
                self.marker2 = self.body2.addMarker(
                    marker('Conn_{}_marker_B'.format(self.name)))
...
class primitivePtPConstraint(constraint):
    ...
    def evaluatePositionConstraintFunction(self, t, y, tDof):
        ...
        '''
        qGlobal = y[:tDof]
        q1 = qGlobal[self.body1.globalDof[:3]] + hf.cardanRotationMatrix(
            qGlobal[self.body1.globalDof[3:]]).dot(self.marker1.position)
        q1 = q1[self.dofB1]
        if self.body2.totalDof != 0:
            q2 = qGlobal[self.body2.globalDof[:3]] + hf.cardanRotationMatrix(
                qGlobal[self.body2.globalDof[3:]]).dot(self.marker2.position)
            q2 = q2[self.dofB2]
        else:
            q2 = np.zeros_like(q1)
...
class hingeJoint(constraint):
    ...
    def evaluatePositionConstraintFunction(self, t, y, tDof):
        ...
        R1 = hf.cardanRotationMatrix(qGlobal[b1dof[3:]])
        R2 = hf.cardanRotationMatrix(
            qGlobal[b2dof[3:]]) if len(b2dof) > 0 else None
    ...
    def evaluateJacobian(self, y):
        ...
        if len(b2dof) != 0:
            ...
            drhoda = hf.cardanRotationMatrixDerivative(
                y[self.body2.globalDof[3:]], 0).dot(rm2)
            drhodb = hf.cardanRotationMatrixDerivative(
                y[self.body2.globalDof[3:]], 1).dot(rm2)
            drhodc = hf.cardanRotationMatrixDerivative(
                y[self.body2.globalDof[3:]], 2).dot(rm2)
...
class nodeEncastreToRigidBody(constraint):
    def __init__(self, name_, flexBody, rigidBody, nodeMarker,
                 rigidMarker=None):
        super().__init__(name_)
        ...
        if rigidMarker == None:
            ...
            orientation = nodeMarker.orientation
            rigidMarker = rigidBody.addMarker(marker('Encastre marker',
                                                     position,
                                                     orientation))
    ...
    def evaluatePositionConstraintFunction(self, t, y, tDof):
        ...
        g = np.hstack((p1 - p2,   # difference in position
                       # difference in orientation vector 1
                       e12 - R2.transpose()[1],
