...
class planarProfile(profile):
    ...
    def __init__(self, name, fileName='', convPar=1):
        ...
        convPar : integer, optional
            Convexity parameter. Used to make spline interpolations. The default is 1.
    ...
    def getCurrentPosition(self):
        """
        Obtain the position of the profile on the parent body reference system.

        Using an affine transformation, returns the position of the profile
        points on the parent body reference system.
        ...
        """
        return (self.points.dot(self.referenceMarker.orientation[0:2, :])
                + self.referenceMarker.position)
...
if __name__ == '__main__':
    ...
    for rSubset in [ch]:
        for wSubset in w.convexSubsets:
            ...
            plt.arrow(pWheel[0], pWheel[1], pRail[0] -
                      pWheel[0], pRail[1]-pWheel[1])
            # print(d)
            if d < minDist:
                minDist = d
